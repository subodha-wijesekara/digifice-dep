...
export function AssignmentDialog({
    ...
    const [modules, setModules] = useState<{ _id: string, name: string, code: string, leader?: string }[]>([]);
            ...
            fetch('/api/admin/modules')
                ...
                .then(data => {
                    if (Array.isArray(data)) {
                        setModules(data);
                        // Pre-select modules assigned to this lecturer
                        const assigned = data.filter((m: any) => m.leader === lecturerId).map((m: any) => m._id);
    ...
    const handleSave = async () => {
        ...
        try {
            // Save Faculty/Dept
            const userRes = await fetch(`/api/users/${lecturerId}`, {
                method: 'PUT',
            ...
            // Save Modules
            const modulesRes = await fetch(`/api/admin/lecturers/assign-modules`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    lecturerId,
            ...
            if (userRes.ok && modulesRes.ok) {
                toast.success("Lecturer assignment updated");
                onSuccess();
                onOpenChange(false);
            } else {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[550px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Update department and module assignments for <strong>{lecturerName}</strong>.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid gap-6 py-4">
                    <div className="space-y-4 border rounded-md p-4 bg-muted/20">
                        <h4 className="font-medium text-sm text-foreground mb-2 flex items-center gap-2">
                            <span className="bg-primary/10 text-primary p-1 rounded">1</span> Department Assignment
                        </h4>
                    ...
                    <div className="space-y-2 border rounded-md p-4 bg-muted/20">
                        <h4 className="font-medium text-sm text-foreground mb-2 flex items-center gap-2">
                            <span className="bg-primary/10 text-primary p-1 rounded">2</span> Module Assignments
                        </h4>
                                        ...
                                        >
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    {module.leader && module.leader !== lecturerId && (
                                                        <span className="text-[10px] bg-yellow-100 text-yellow-700 px-1 rounded">Assigned to other</span>
                                                    )}
                                                </div>
                        ...
                        <p className="text-[10px] text-muted-foreground">
                            *Modules marked "Assigned to other" will be reassigned to this lecturer if selected.
                        </p>
                    </div>
                </div>
